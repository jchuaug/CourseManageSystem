...
function getId(){
	...
	var index2= url.indexOf("toSeminarRandom/");
	courseId= url.substring(index1+7,index2-1);
	seminarId=url.substring(index2+16);
}
	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i = 0; i < data.length; i++) {
				...
				topic.innerHTML = "<div hidden='hidden'>" + data[i].id + "</div><div class='BlockFont'>" + data[i].name + "</div>";
...
function toTopic(topic){
	...
	window.location.href = "/course/"+courseId+"/topicRandom/"+topicId;
}
